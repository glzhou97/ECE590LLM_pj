...
from fast_attention_test import kernel_feature_creator  # note this is the sin cos matrix
from fast_attention import nonnegative_softmax_kernel_feature_creator  # this is the non-negative one 
import numpy as np
...
def Exp2():
    ...
    for nb_random_features in random_feature_range:
        ...
        for b in range(0, batch_size): 
            sin_cos_error = (standard_attention_result[b] - sin_cos_rfm_attention_result[b])**2
            nonnegative_error = (standard_attention_result[b] - nonnegative_rfm_attention_result[b])**2
    ...
    plt.plot(random_feature_range, sin_cos_errors, label='Sin/Cos Features', color='red')
    plt.plot(random_feature_range, nonnegative_errors, label='Nonnegative Features', color='blue')
    plt.fill_between(random_feature_range, np.array(sin_cos_errors)-np.array(sin_cos_errors_std), np.array(sin_cos_errors)+np.array(sin_cos_errors_std), color='red', alpha=0.3)
    plt.fill_between(random_feature_range, np.array(nonnegative_errors)-np.array(nonnegative_errors_std),np.array(nonnegative_errors)+np.array(nonnegative_errors_std), color='blue', alpha=0.3)
