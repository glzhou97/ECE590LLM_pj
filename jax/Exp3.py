...
def Exp3():
    ...
    for nb_random_features in random_feature_range:
        ...
        for b in range(0, batch_size): 
            sm_error = (standard_attention_result[b] - sm_rfm_attention_result[b])**2
            hypo_error = (standard_attention_result[b] - hypo_rfm_attention_result[b])**2
    ...
    plt.plot(random_feature_range, sm_errors, label='SM Features', color='red')
    plt.plot(random_feature_range, hypo_errors, label='SM^hypo Features', color='blue')
    plt.fill_between(random_feature_range, np.array(sm_errors)-np.array(sm_errors_std), np.array(sm_errors)+np.array(sm_errors_std), color='red', alpha=0.3)
    plt.fill_between(random_feature_range, np.array(hypo_errors)-np.array(hypo_errors_std),np.array(hypo_errors)+np.array(hypo_errors_std), color='blue', alpha=0.3)
